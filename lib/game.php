...
function check_abort() {
	...
	$sql = "update game_status set status='aborded', result=if(p_turn='C','P','C'),p_turn=null where p_turn is not null and last_change<(now()-INTERVAL 5 MINUTE) and status='started'";
	$st = $mysqli->prepare($sql);
	$r = $st->execute();
}
	...
	switch($active_players) {
		...
		case 2: $new_status='started'; 
				if($status['p_turn']==null) {
					$new_turn='P'; // It was not started before...
				}
				break;
	}
